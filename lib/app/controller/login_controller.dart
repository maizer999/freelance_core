...
class LoginController extends GetxController implements GetxService {
  ...
  Future<void> onLogin() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (myMap['user'] != '' && myMap['token'] != '' && myMap['user']['type'] == 'freelancer') {
        debugPrint(myMap['user']['id'].toString());
        parser.saveUID(myMap['user']['id'].toString());
        parser.saveToken(myMap['token']);
        parser.saveData(myMap['user']['cover'].toString(), myMap['user']['first_name'].toString(), myMap['user']['last_name'].toString(), myMap['user']['mobile'].toString(), myMap['user']['email'].toString());
    ...
    if (smsName == '2') {
      ...
      if (response.statusCode == 200) {
        ...
        if (myMap['data'] != '' && myMap['data'] == true) {
          ...
          Get.toNamed(AppRouter.getFirebaseVerificationRoutes(), arguments: [countryCode, mobileNo.text, 'login']);
        } else {
