...
import 'package:freelancer/app/backend/model/chat_list_model.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> sendMessage() async {
    ...
    yourMessage = true;
    update();
    var param = {'room_id': roomId, 'uid': uid, 'sender_id': uid, 'message': msg, 'message_type': 0, 'reported': 0, 'status': 1};
    Response response = await parser.sendMessage(param);
    yourMessage = false;
    update();
    if (response.statusCode == 200) {
      var notificationParam = {"id": receiverId, "title": 'New message received', "message": message};
