  ...
  return Container(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: <BoxShadow>[BoxShadow(color: ThemeProvider.blackColor.withValues(alpha: 0.2), offset: const Offset(0, 1), blurRadius: 3)],
      ),
      child: TextField(
        onChanged: (String txt) {},
        style: const TextStyle(fontSize: 16, fontFamily: 'medium'),
        cursorColor: ThemeProvider.appColor,
        decoration: InputDecoration(border: InputBorder.none, hintText: hint, hintStyle: const TextStyle(color: ThemeProvider.greyColor, fontSize: 14)),
      ),
    ),
  );
}
...
smallText(val) {
  return Text(val, style: const TextStyle(fontSize: 11, color: ThemeProvider.greyColor));
}

colorText(val) {
  return Text(val, style: const TextStyle(fontSize: 22, fontFamily: 'bold', color: ThemeProvider.appColor));
}
...
myBoxDecoration() {
  return BoxDecoration(
    ...
    boxShadow: [BoxShadow(color: ThemeProvider.blackColor.withValues(alpha: 0.2), offset: const Offset(0, 1), blurRadius: 3)],
  );
}
